...
class ViewController: UIViewController {
    var comScore = 0 {
        didSet {
            ...
            return comScoreLabel.text = "컴퓨터: \(comScore)"
        }
    }
    var userScore = 0 {
        didSet {
            ...
            userScoreLabel.text = "사용자: \(comScore)"
        }
    }
    ...
    @objc func rpsButtonTapped(sender: UIButton) {
        ...
        guard let myChoice = sender.titleLabel?.text,
              let myRps = Rps(rawValue: myChoice),
              let comChoice = comRPSLabel.text,
              let comRps = Rps(rawValue: comChoice) else { return }
        let resultString = rpsResult(value1: myRps, value2: comRps)
        ...
        print(rpsResult(value1: myRps, value2: comRps))
